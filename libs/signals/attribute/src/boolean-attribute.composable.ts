 ...
 */
export const useBooleanAttribute = (attributeName: string, options?: IUseBooleanAttributeOptions): WritableSignal<boolean | undefined> => {
    const { namespace, initialValue, defaultValue } = normalizeUseBooleanAttributeOptions(options);

    const value = signal<boolean | undefined>(initialValue);

    return bindBooleanAttribute(attributeName, value, { namespace, defaultValue });
}
 ...
 */
export const bindBooleanAttribute = <T extends Signal<boolean | undefined>>(attributeName: string, value: T, options?: IBindBooleanAttributeOptions) => {
    const { namespace, defaultValue } = normalizeBindBooleanAttributeOptions(options);

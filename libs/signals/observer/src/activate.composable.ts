...
export const useActivate = (options: IUseActivateOptions) => {
    ...
    const keydown$ = fromEvent<KeyboardEvent>(target, 'keydown').pipe(filter((event) => options.keydown?.includes(event.key) ?? false));
...
export const useActivateSignal = (options: IUseActivateOptions) => {
    ...
    return computed(() => {
        ...
        if (options.keydown && keydown() && options.keydown.includes((keydown() as KeyboardEvent)?.key)) {
            return keydown();
        }
